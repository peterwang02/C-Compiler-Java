...
if_stmt returns [IfStatementNode node]: ('if' '(' cond ')' '{' s1=statements '}' ('else' '{' s2=statements '}')
                                        {$node = new IfStatementNode($cond.node, $s1.node, $s2.node);}) |
                                        ('if' '(' cond ')' '{' s1=statements '}' (/* empty */)
                                        {$node = new IfStatementNode($cond.node, $s1.node, null);});
/*  
...
array_expr returns [PtrDerefNode node] : lval '[' expr ']' {ExpressionNode f1 = new IntLitNode("4"); ExpressionNode f2 = new BinaryOpNode($expr.node, f1, "*"); 
                                                            ExpressionNode f3 = new BinaryOpNode($lval.node, f2, "+"); $node = new PtrDerefNode(f3);} /* FILL IN FOR STEP 6 */
        | ae=array_expr '[' expr ']'{$node = new PtrDerefNode($ae.node);}; /* FILL IN FOR STEP 6 */
...
type_cast returns [TypeCastNode node] : '(' base_type ')' primary {$node = new TypeCastNode($base_type.t, $primary.node);};
